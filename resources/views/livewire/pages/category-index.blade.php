<div style="min-height: 100vh; background-color: #f3f4f6;">
    <div style="max-width: 80rem; margin-left: auto; margin-right: auto; padding-left: 1rem; padding-right: 1rem; padding-top: 3rem; padding-bottom: 3rem;">
        <div style="text-align: center; margin-bottom: 3rem;">
            <h1 style="font-size: 2.25rem; font-weight: 700; color: #111827; margin-bottom: 1rem;">{{ __('categories.title') }}</h1>
            <p style="font-size: 1.25rem; color: #6b7280;">{{ __('categories.subtitle') }}</p>
        </div>
                    ...
                    <div style="background-color: white; border-radius: 0.5rem; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); border: 1px solid #e5e7eb; padding: 1.5rem;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
                            ...
                            <span style="background-color: #dbeafe; color: #1e40af; font-size: 0.75rem; font-weight: 500; padding: 0.25rem 0.5rem; border-radius: 9999px;">
                                {{ $category->designPatterns->count() }} {{ __('categories.patterns') }}
                            </span>
                        </div>
                        
                        @if($category->description_zh)
                            <p style="color: #6b7280; margin-bottom: 1rem; line-height: 1.5;">{{ $category->description_zh }}</p>
                        @endif

                        <div style="display: flex; justify-content: space-between; align-items: center; padding-top: 1rem; border-top: 1px solid #f3f4f6;">
                            <span style="font-size: 0.875rem; color: #6b7280;">{{ __('categories.category') }}</span>
        ...
        @else
            <div style="text-align: center; padding: 3rem;">
                ...
                <h3 style="font-size: 1.5rem; font-weight: 600; color: #111827; margin-bottom: 0.5rem;">{{ __('categories.no_categories_found') }}</h3>
                <p style="color: #6b7280;">{{ __('categories.no_categories_created') }}</p>
            </div>
        @endif
    </div>
</div>